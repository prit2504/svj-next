...
export default function CategoryPage() {
  ...
  // 🔹 Load category state when component mounts
  useEffect(() => {
    const savedState = JSON.parse(sessionStorage.getItem(`category-${decodedCategory}`));
    ...
    if (savedState) {
      ...
      setTimeout(() => {
        window.scrollTo({ top: savedState.scrollY || 0 });
      }, 100);
    } else {
      fetchProducts(1);
    }
  ...
  // 🔹 Save scroll position and state
  useEffect(() => {
    const handleScroll = () => {
  ...
  // 🔹 Navigate to product detail (and save scroll first)
        ...
        {loading ? (
          ...
          products.map((product) => (
            <button
              ...
              className="group product-card flex flex-col overflow-hidden rounded-2xl shadow-md border border-[#f5e6d3] bg-gradient-to-br from-[#fffdf9] via-[#fff8f2] to-[#fff3ea] hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer"
            >
              {/* Image */}
              <div className="relative overflow-hidden w-full aspect-square">
                <img
