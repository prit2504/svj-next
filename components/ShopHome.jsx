...
export default function ShopPage() {
  ...
  const limit = 16;
  const STORAGE_KEY = "shopPageState";

  // -----------------------------------------------------
  // SCROLL POSITION + STATE SAVE
  // -----------------------------------------------------
  const saveScrollPosition = useCallback(
    debounce(() => {
      const state = JSON.parse(sessionStorage.getItem(STORAGE_KEY)) || {};
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setSearchTriggered(saved.searchTriggered || false);
      setShowAll(saved.showAll || false);
  ...
  const saveState = useCallback(
    ...
    [selectedCategory, searchTerm, products, page, pages, searchTriggered, showAll]
  );

  useEffect(() => {
    saveState();
    return () => saveState.cancel();
  }, [selectedCategory, searchTerm, products, page, pages, searchTriggered, showAll, saveState]);
      ...
      try {
        const res = await fetch(
          `/api/products?category=${encodeURIComponent(
            selectedCategory
          )}&search=${encodeURIComponent(searchTerm)}&page=${currentPage}&limit=${limit}`
        );
        const data = await res.json();
        productCache.current[cacheKey] = data;
        setProducts(data.products);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const data = await res.json();
        const formatted = Object.entries(data.arrivals).map(([cat, prods]) => ({
          category: cat,
          products: prods,
        }));
        setNewArrivals(formatted);
        sessionStorage.setItem("newArrivals", JSON.stringify(formatted));
      } catch (err) {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <section className="max-w-7xl mx-auto px-4 py-10 bg-gradient-to-r from-[#fffaf5] to-[#fdf3e7] rounded-2xl shadow-xl border border-red-300 relative">
        ...
        {loadingArrivals ? (
          ...
          <>
            <div
              className={`space-y-12 transition-all duration-500 ${showAll ? "max-h-full" : "max-h-[1200px] overflow-hidden relative"
                }`}
            >
              {(showAll ? newArrivals : newArrivals.slice(0, 3)).map(
                (catData, i) =>
                  catData.products.length > 0 && (
                    <div key={i}>
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <button
                          onClick={() => router.push(`/category/${encodeURIComponent(catData.category)}`)}
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                        {(isMobile
                          ? catData.products.slice(0, 2)
                          : catData.products.slice(0, 4)
                        ).map((product) => (
                          <div
                            key={product._id}
                            onClick={() => router.push(`/product/${product._id}`)}
                            className="group cursor-pointer flex flex-col overflow-hidden rounded-2xl shadow-md border border-[#f5e6d3] bg-gradient-to-br from-[#fffdf9] via-[#fff8f2] to-[#fff3ea] hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
                          >
                            <div className="relative overflow-hidden w-full aspect-square">
                              <img
                                src={optimizeCloudinaryUrl(product.imageUrl, 400, 400)}
                            ...
                            <div className="p-4 flex flex-col flex-1 font-sans">
                              ...
                              <div className="border-t border-gray-200 pt-3 mt-2 text-sm space-y-1">
      ...
      <section
        ...
        className="bg-gradient-to-r from-[#e2b7bd] to-[#dac6aa] rounded-2xl p-3 shadow-xl border border-red-300 mt-10"
      >
        {products.length === 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-2 gap-4 px-4 md:px-8">
            <div className="flex justify-center">
              <img
                src="/images/htu1.webp"
                alt="How to Use 1"
                className="w-full max-w-sm h-auto rounded-lg shadow-md object-cover hover:scale-105 transition-transform duration-300"
              />
            </div>
            <div className="flex justify-center">
              <img
                src="/images/htu2.webp"
                alt="How to Use 2"
                className="w-full max-w-sm h-auto rounded-lg shadow-md object-cover hover:scale-105 transition-transform duration-300"
              />
            </div>
          ...
          {loading ? (
            ...
            products.map((product) => (
              <button
                key={product._id}
                onClick={() => router.push(`/product/${product._id}`)}
                className="group flex flex-col overflow-hidden rounded-2xl shadow-md border border-[#f5e6d3] bg-gradient-to-br from-[#fffdf9] via-[#fff8f2] to-[#fff3ea] hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer"
              >
                <div className="relative overflow-hidden w-full aspect-square">
                  <img
                    src={optimizeCloudinaryUrl(product.imageUrl, 400, 400)}
